-- BATCH 3 OF 4: Migrations 00024-00039 (SAFE)
...
DO $$ BEGIN
  ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS premium_tier integer DEFAULT 0;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE memorials ADD COLUMN IF NOT EXISTS biography_is_ai_generated boolean DEFAULT false;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE memorials ADD COLUMN IF NOT EXISTS place_of_birth text;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'memorials' AND column_name = 'place_of_death'
  ) THEN
    ALTER TABLE memorials ADD COLUMN IF NOT EXISTS place_of_death text;
  END IF;
END $$;
...
-- Wedding pages
CREATE INDEX IF NOT EXISTS idx_wedding_pages_partner1 ON wedding_pages(partner1_user_id);
CREATE INDEX IF NOT EXISTS idx_wedding_pages_partner2 ON wedding_pages(partner2_user_id);
CREATE INDEX IF NOT EXISTS idx_wedding_pages_slug ON wedding_pages(slug);
CREATE INDEX IF NOT EXISTS idx_wedding_pages_status ON wedding_pages(status);
CREATE INDEX IF NOT EXISTS idx_wedding_pages_date ON wedding_pages(wedding_date);

-- Wedding RSVPs
CREATE INDEX IF NOT EXISTS idx_wedding_rsvps_wedding ON wedding_rsvps(wedding_id);
CREATE INDEX IF NOT EXISTS idx_wedding_rsvps_user ON wedding_rsvps(user_id);
CREATE INDEX IF NOT EXISTS idx_wedding_rsvps_status ON wedding_rsvps(status);

-- Wedding party
CREATE INDEX IF NOT EXISTS idx_wedding_party_wedding ON wedding_party(wedding_id);
CREATE INDEX IF NOT EXISTS idx_wedding_party_user ON wedding_party(user_id);

-- Wedding messages
CREATE INDEX IF NOT EXISTS idx_wedding_messages_wedding ON wedding_messages(wedding_id);
CREATE INDEX IF NOT EXISTS idx_wedding_messages_author ON wedding_messages(author_id);

-- Pet pages
CREATE INDEX IF NOT EXISTS idx_pet_pages_created_by ON pet_pages(created_by);
CREATE INDEX IF NOT EXISTS idx_pet_pages_slug ON pet_pages(slug);
CREATE INDEX IF NOT EXISTS idx_pet_pages_species ON pet_pages(species);
CREATE INDEX IF NOT EXISTS idx_pet_pages_status ON pet_pages(status);

-- Pet milestones
CREATE INDEX IF NOT EXISTS idx_pet_milestones_pet ON pet_milestones(pet_id);
CREATE INDEX IF NOT EXISTS idx_pet_milestones_type ON pet_milestones(milestone_type);

-- Pet tributes
CREATE INDEX IF NOT EXISTS idx_pet_tributes_pet ON pet_tributes(pet_id);
CREATE INDEX IF NOT EXISTS idx_pet_tributes_author ON pet_tributes(author_id);
