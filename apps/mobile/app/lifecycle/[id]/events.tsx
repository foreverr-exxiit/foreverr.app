        ...
        <View className="mx-4 mb-3 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 overflow-hidden">
          ...
          <View className="p-4">
            ...
            <View className="mt-3 flex-row flex-wrap items-center gap-x-4 gap-y-2">
              ...
              {/* Ticketed badge */}
              {item.is_ticketed && (
                <View className="flex-row items-center bg-brand-50 dark:bg-brand-900/20 rounded-full px-2.5 py-0.5">
                  <Ionicons name="ticket" size={12} color="#4A2D7A" />
                  <Text className="ml-1 text-[11px] font-sans-semibold text-brand-700">
                    {item.ticket_price_cents === 0
                      ? "Free Ticket"
                      : `$${(item.ticket_price_cents / 100).toFixed(2)}`}
                  </Text>
                  {item.ticket_limit && (
                    <Text className="ml-1 text-[10px] font-sans text-gray-400">
                      · {item.tickets_sold ?? 0}/{item.ticket_limit}
                    </Text>
                  )}
                </View>
              )}
