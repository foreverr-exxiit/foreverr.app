...
export default function TimelineScreen() {
  ...
  if (memorial?.date_of_birth) {
    timelineItems.push({
      ...
      label: config.timelineLabels.born,
      icon: "star" as const,
      color: "#4A2D7A",
  ...
  // "Passed Away" — only in memorial mode
  if (config.timelineLabels.passed && memorial?.date_of_death) {
    timelineItems.push({
      date: memorial.date_of_death,
      label: config.timelineLabels.passed,
  ...
  if (memorial?.created_at) {
    timelineItems.push({
      ...
      label: config.timelineLabels.created,
    ...
    <ScrollView className="flex-1" contentContainerStyle={{ flexGrow: 1, paddingBottom: 32 }}>
      {timelineItems.length > 0 ? (
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="mt-3 text-lg font-sans-bold text-gray-900 dark:text-white text-center">The Arc</Text>
          <Text className="mt-1 text-sm font-sans text-center text-gray-500">
            {config.timelineLabels.emptyState}
          </Text>
        </View>
      )}
    </ScrollView>
